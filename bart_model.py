...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and use_importance and importance is not None:
            importance = importance.repeat(bsz * self.num_heads, tgt_len, 1)
            importance = nn.functional.softmax(importance, dim=-1)
            # logger.warning("************importance works************")
            assert attn_weights.shape == importance.shape
            attn_weights = (1-self.alpha)*attn_weights + self.alpha*importance
            # attn_weights = torch.mul(attn_weights, importance)
            # attn_weights = nn.functional.softmax(attn_weights, dim=-1)
...
class BartDecoderLayer(nn.Module):
    def __init__(self, cur_layer_id, config: BartConfig):
        super().__init__()
        self.embed_dim = config.d_model
        ...
        self.final_layer_norm = nn.LayerNorm(self.embed_dim)
        self.use_importance = True if config.layerid==cur_layer_id else False
...
class BartDecoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([BartDecoderLayer(i+1, config) for i in range(config.decoder_layers)])
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        ...
                        return module(*inputs, output_attentions, use_cache, importance=importance)
